...
if [ "$EXT" = "fpc" ]; then
    ...
    if tr -d ' \t\n\r\f' < code.pas | grep -q 'assign('; then
            shj_log "assign is not allowed"
			echo 'code.pas:assign is not allowed' >cerr
			EXITCODE=110
            shj_finish "CE"
    fi
    if tr -d ' \t\n\r\f' < code.pas | grep -q 'deletefile('; then
            shj_log "deletefile is not allowed"
			echo 'code.pas:assign is not allowed' >cerr
			EXITCODE=110
            shj_finish "CE"
    fi
    if tr -d ' \t\n\r\f' < code.pas | grep -q 'erase('; then
            shj_log "erase is not allowed"
			echo 'code.pas:assign is not allowed' >cerr
			EXITCODE=110
            shj_finish "CE"
    fi
...
for((i=1;i<=TST;i++)); do
	...
	if [ "$EXT" = "java" ]; then
		...
		if $PERL_EXISTS; then
			...
			# shj_log "./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/in/input$i.txt 'python2 -O $FILENAME.py'"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/in/input$i.txt "python2 -O $FILENAME.py"
		fi
		EXITCODE=$?
	...
	if $ACCEPTED; then
		shj_log "ACCEPTED"
		echo "<span class=\"shj_g\">ACCEPTED ($t sec)</span>" >>$PROBLEMPATH/$UN/result.html
		((PASSEDTESTS=PASSEDTESTS+1))
	else
		shj_log "WRONG"
		ACCEPTEDALL=false
		echo "<span class=\"shj_r\">WRONG ($t sec)</span>" >>$PROBLEMPATH/$UN/result.html
	fi
done
...
if $ACCEPTEDALL; then
	((SCORE=10000)) # give score from 10,000
else
	((SCORE=0))
fi
